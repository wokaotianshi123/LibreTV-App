...
async function search() {
    ...
    if (!query) {
        if (typeof showToast === 'function') showToast('请输入搜索内容', 'info'); else console.warn('showToast not defined');
        return;
    }
    
    if (selectedAPIs.length === 0) { // selectedAPIs from app_globals.js
        if (typeof showToast === 'function') showToast('请至少选择一个API源', 'warning'); else console.warn('showToast not defined');
        return;
    }
    
    if (typeof showLoading === 'function') showLoading(); else console.log("Loading..."); // Assuming showLoading is global
    
    try {
        // 保存搜索历史
        // Assuming saveSearchHistory is global or defined elsewhere (e.g., history.js)
        if (typeof saveSearchHistory === 'function') saveSearchHistory(query); else console.warn("saveSearchHistory not defined");
        sessionStorage.setItem('lastSearchQuery', query);
        sessionStorage.setItem('lastPageView', 'searchResults');
        
        let allResults = [];
        const searchPromises = selectedAPIs.map(async (apiId) => { // selectedAPIs from app_globals.js
            let apiUrl = ''; 
            try {
                ...
                if (apiId.startsWith('custom_')) {
                    ...
                    // API_CONFIG assumed to be global or defined elsewhere
                    apiUrl = apiBaseUrl + API_CONFIG.search.path + encodeURIComponent(query);
                    apiName = customApi.name;
                } else {
                    // API_SITES assumed to be global
                ...
                // AGGREGATED_SEARCH_CONFIG assumed to be global
                ...
                const results = data.list.map(item => ({
                    ...
                    source_name: apiName,
                    source_code: apiId,
                    api_url: apiId.startsWith('custom_') ? (typeof getCustomApiInfo === 'function' ? getCustomApiInfo(apiId.replace('custom_', ''))?.url : undefined) : undefined
                }));
        ...
        if (!allResults || allResults.length === 0) {
            ...
            `;
            if (typeof hideLoading === 'function') hideLoading(); else console.log("Loading complete."); // Assuming hideLoading is global
            return;
        }
        ...
        const safeResults = allResults.map(item => {
            ...
            const sourceInfo = item.source_name ? 
                `<span class="bg-[#222] text-xs px-1.5 py-0.5 rounded-full">${item.source_name.replace(/</g, '<').replace(/>/g, '>')}</span>` : '';
            ...
            // showDetails is expected to be in details_modal.js
            return `
        ...
        if (error.name === 'AbortError') {
            if (typeof showToast === 'function') showToast('搜索请求超时，请检查网络连接', 'error'); else console.error('Search timed out');
        } else {
            if (typeof showToast === 'function') showToast('搜索请求失败，请稍后重试', 'error'); else console.error('Search failed');
        }
    } finally {
        if (typeof hideLoading === 'function') hideLoading(); else console.log("Loading complete."); // Assuming hideLoading is global
    }
